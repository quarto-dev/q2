...
### The eight entry points vs. Q2's internal boundaries

Layer 2 said Q1's eight entry points collapse onto just `pre`/`post`. But that collapse is a
*choice about what to expose*, not a limit of the pipeline: **every one of Q1's eight entry
points corresponds to a boundary that already exists in Q2** — a phase boundary inside
`AstTransformsStage`, or a gap between macro stages. Q2 surfaces only two of them today. This
is the menu to consult if we ever decide to expose **more than one** new Lua-filter entry
point.

| Q1 entry point | Q1 meaning | Q2 internal boundary (all real, already there) | Exposed today? |
|---|---|---|---|
| `pre-ast` | raw AST, before Quarto touches it | after `ParseDocumentStage` | ❌ |
| `post-ast` | after normalize sugar established | end of **Normalization** phase | ❌ |
| `pre-quarto` | before semantic pre-processing | start of `AstTransformsStage` | ✅ = current `pre` |
...
`FilterPosition` variant (`user_filters.rs:17-23`), split `AstTransformsStage` at that phase
boundary, and extend the `ENTRY_POINTS` table (`filter_resolve.rs:33-40`) to route the Q1
name there. The extensions plan itself flagged this path ("finer-grained positions can be
added later by splitting `AstTransformsStage`"). The `PostCrossref` seam (`post-quarto`, end
of Crossref) is the highest-value single addition — see the synthesis.
