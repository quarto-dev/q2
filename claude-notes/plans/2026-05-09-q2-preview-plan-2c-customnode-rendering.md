...
- [x] **4.4** `q2-preview/dispatchers.tsx` extension + `q2-preview/registry.ts` extension + `entry.tsx` PreviewRoot rename — add `CustomBlock` / `CustomInline` dispatcher components to `dispatchers.tsx` (sibling of the existing `Block` / `Inline`); both look up `node.type_name` from the framework's existing `RegistryContext`, falling back to `__fallback__`. **Tiny tag-along edit to existing `Block` / `Inline` placeholders**: their miss-path JSX currently uses an inline `placeholderStyle` object (`color: '#888'`, italic) but no class name; add `className="q2-preview-placeholder"` alongside the existing `style={placeholderStyle}` so the must-not-match smoke-fixture selector (`div.q2-preview-placeholder`) actually matches when the placeholder fires. ~2 LOC, localized to `dispatchers.tsx`. Spread `Custom` exports + `__fallback__: Fallback` into `previewRegistry` alongside the Pandoc-tag entries — the namespace is disjoint by policy. Rename `mergedRegistry` → `mergedPreviewRegistry` in `entry.tsx` (1-line change, around line 228) for clarity now that the same map carries both Pandoc-tag and CustomNode entries. **`framework/types.ts` `FormatRegistry` is NOT extended** — the required-key intersection stays at `{ Ast; Block; Inline }`. `CustomBlock` / `CustomInline` enter `previewRegistry` as untyped keys (the existing `Record<string, ...>` index signature accepts them). Reason: `q2DebugRegistry` (`q2-debug/registry.ts`) also satisfies `FormatRegistry` and does not register these keys — making them required would force q2-debug to ship CustomBlock/CustomInline dispatchers that duplicate its existing "Not registered: <tag>" fallback path. The runtime presence of these keys in `previewRegistry` is locked by the §"Class-compatibility test" + the smoke-fixture `must_not_match` selector for `div.q2-preview-placeholder` — both fail loudly if the keys go missing. After 4.4 lands, the muted-gray "(not yet implemented)" miss path stops firing for Pandoc CustomNode wrappers, and user TSX can override Pandoc tags (already supported) or CustomNode `type_name`s — both flow through the single existing merge site, no second context. Includes vitest integration test for both override directions (Pandoc tag override + CustomNode override) and a unit test at the `customRegistry`-build layer (per #12 — locks the merge contract one layer below the smoke fixture).
...
- [x] **5.2** Smoke-all q2-preview fixtures — under `crates/quarto/tests/smoke-all/q2-preview/` (directory exists post-2B's `image-with-attrs.qmd` fixture; 2C extends it):
...
The six concrete `type_name` strings (canonical source: `crates/quarto-core/src/crossref/mod.rs:60-92` plus the `CustomNode::new("Callout", ...)` site at `crates/quarto-core/src/transforms/callout.rs:233`):
...
- **Output structure** (mirroring `callout_resolve.rs:170-234`, q2-preview must emit identically because `callout-resolve` is excluded from the pipeline so the CustomNode survives — see `Q2_PREVIEW_TRANSFORM_EXCLUDED` at `pipeline.rs:1050`):
  ...
  **Identifier — omit if empty** (same rule as Theorem / FloatRefTarget). The user's attr id flows through to the wrapper's `id`; an unannotated callout (`::: {.callout-note}` with no id) ends up with `node.attr[0] === ""`, and the wrapper must not emit `id=""`. Match Rust behavior — the Rust `Div` writer skips empty ids. The worked example's `id={node.attr[0] || undefined}` encodes this.
...
The `FormatRegistry` type from `framework/types.ts:163` enforces `Ast` / `Block` / `Inline` as required keys. **2C does NOT extend the required-key set** — `CustomBlock` / `CustomInline` flow in via the existing `Record<string, (props: any) => React.ReactNode>` index signature. Reason: `q2DebugRegistry` also satisfies `FormatRegistry` and does not (and need not) register these keys; making them required would force q2-debug to ship CustomBlock/CustomInline dispatchers that duplicate its "Not registered: <tag>" fallback. q2-preview's invariant "the keys are present in `previewRegistry`" is locked at the runtime layer (§Test plan "Class-compatibility test" + smoke-fixture `must_not_match` for `div.q2-preview-placeholder`). The rest of the keys are still loosely typed since CustomNode `type_name`s are dynamic.
...
- **Atomic CustomNode read-only — gate inherited from Plan 2B.** The framework's atomic gate (Plan 2B Phase 1.3) detects atomic CustomNode types via `isAtomicCustomNode(node.type_name)` and no-ops `setLocalAst` for them. **Gate location**: `framework/dispatch.tsx:407-411` — the no-op wrap (`effectiveSetLocalAst = isAtomic ? NOOP_SET_LOCAL_AST : setLocalAst`) happens **inside `<Node>` before format-registry dispatch**, so per-type components like `CrossrefResolvedRef.tsx` receive an already-no-op'd `setLocalAst` from the framework — no per-component awareness needed. Same pattern for Plan 8's `IncludeExpansion` once it lands and updates `atomicCustomNodes.ts`.
...
- **Namespace-disjoint policy assertion**: build-time vitest that asserts every `Custom.*` export name (`Callout`, `Theorem`, `Proof`, `FloatRefTarget`, `Equation`, `CrossrefResolvedRef`) is disjoint from the canonical Pandoc tag set. Pattern mirrors Rust's `pipeline.rs:1987` (`Q2_PREVIEW_TRANSFORM_EXCLUDED` validation) — a static-set-vs-static-set assertion that runs at every build and catches silent shadowing if a future Quarto transform picks a `type_name` matching a Pandoc tag. **The Pandoc tag set is maintained as a hardcoded list in the test file** (`framework/types.ts` does not currently export one — `BlockNode` / `InlineNode` are TS unions, not introspectable at runtime). The list covers the ~30 tag names from `framework/types.ts`'s `BlockNode` and `InlineNode` unions: `Para, Plain, Header, BlockQuote, OrderedList, BulletList, DefinitionList, RawBlock, HorizontalRule, Table, Div, CodeBlock, LineBlock, Figure, CustomBlock, Str, Emph, Underline, Strong, Strikeout, Superscript, Subscript, SmallCaps, Quoted, Cite, Code, Space, SoftBreak, LineBreak, Math, RawInline, Link, Image, Note, Span, CustomInline`. The test owns the list (it's the consumer); a comment pins the source `framework/types.ts:60+ / :111+` so updates stay in sync if Pandoc adds new tags. ~40 LOC total (list + assertion); lives in a new `q2-preview/registry.test.ts` next to `registry.ts`. The §Design decisions §"Unified registry, namespace-disjoint by policy" entry is the rule this test enforces.
  ...
  tests:
    ...
    q2-preview:
      ensureHtmlElements:
        ...
        - # must-not-match selectors
          - 'div.q2-preview-placeholder'  # framework muted-gray "(not yet implemented)"
          - 'div[data-custom-type]'       # raw CustomNode wrapper not unwrapped
---
```

`ensureHtmlElements` is **two arrays of CSS selectors**: the first array's selectors must all be present in the iframe DOM; the second array's selectors must all be absent. The runner contract is at `parse_ensure_html_elements` in `crates/quarto/tests/smoke-all/spec.rs` (`[must_match, must_not_match]`; second array optional). The must-not-match assertions above lock two invariants: (a) every CustomNode resolved to a real component (no fallback placeholder), and (b) the framework's `unwrapCustomNodes` walk produced typed CustomBlock/CustomInline nodes (no raw `data-custom-type` wrapper Divs/Spans leaked into the rendered DOM).

Element-and-structure selectors (e.g. `div.callout-note > div.callout-header > div.callout-title-container.flex-fill`) lock the three-deep nesting rule from §"`Callout.tsx`" — flattening any level breaks descendant-combinator CSS rules and these selectors catch it.
...
- **`q2-preview/multi-element-project/`** (project-mode) — directory containing `_quarto.yml` (minimal `project:\n  title: ...` — no `type:` key, so it's the default project type, not a website) + `index.qmd` mirroring the multi-element-doc content + a sibling `notes.qmd` so the orchestrator's pass-1 indexes more than one file. Same `ensureHtmlElements` assertions as `multi-element-doc.qmd` (only `index.qmd` carries the assertion frontmatter; `notes.qmd` is filler for pass-1 multiplicity). **This fixture is what enforces the project-context coverage rule for 2C's smoke layer** — without it the project pass-2 renderer path (`pass2_renderer.rs::RenderToPreviewAstRenderer`) goes untested for CustomNode rendering.
  ...
  **`notes.qmd` content — pin to this exact stub** so the fixture is deterministic across re-runs:
  ...
  No `format:` key (inherits from `_quarto.yml`'s default), no `_quarto.tests` (assertions only land on `index.qmd`). Three lines of body is enough for the orchestrator to count it as a real document.
...
- `~/docs/demo-playground/elliot/{html,kanban,comment,simple,drag,slide}.tsx` — fork target. After 2B+2C, pruned forks land at `~/docs/demo-playground/gordon/render-components/`.
...
- **2026-05-10 (fourth-pass: review nits + spec tightening)**: pre-implementation review against current sources turned up two cosmetic mismatches and several underspecified spots. No design changes. Source claims, line refs, and Plan 2B prerequisites were all re-verified against HEAD on `feature/q2-preview` and pass; the items below are doc-only.

  **Cosmetic fixes**:
  - "seven concrete `type_name` strings" → "six" (off-by-one residue from before the third-pass `IncludeExpansion` deferral).
  - References list pruned `custom.tsx` from `~/docs/demo-playground/elliot/{...}.tsx` — that file does not exist in the demo-playground tree (only `html, kanban, comment, simple, drag, slide`).

  **Spec tightening** (no behavior change; pinning invariants the prior text glossed over):
  - **Callout `flex-fill` line ref**: cited `callout_resolve.rs:215` (`make_attr(&["callout-title-container", "flex-fill"])`) so the "mandatory" claim is drift-detectable like the other class-name refs.
  - **Callout identifier-omit-if-empty rule**: added explicitly (mirroring Theorem / FloatRefTarget). The worked example's `id={node.attr[0] || undefined}` already encoded it; the per-component spec now states the rule.
  - **Phase 4.4 framework type change reconsidered and dropped** (replaces an earlier intra-pass version that proposed extending `FormatRegistry`). Cross-checking turned up `q2DebugRegistry` at `hub-client/src/components/render/q2-debug/registry.ts:18`, which also satisfies `FormatRegistry` and intentionally does NOT register `CustomBlock` / `CustomInline` (q2-debug routes those through its bordered "Not registered: <tag>" fallback at `q2-debug/dispatchers.tsx`). Making the keys required at the type level would force q2-debug to ship dispatchers that duplicate that fallback path. **Resolution**: `FormatRegistry`'s required-key intersection stays at `{ Ast; Block; Inline }`; `CustomBlock` / `CustomInline` enter `previewRegistry` via the existing `Record<string, …>` index signature. Runtime presence is locked by §Test plan "Class-compatibility test" + smoke-fixture `must_not_match` assertion on `div.q2-preview-placeholder`. Phase 4.4 narrative + §"`q2-preview/dispatchers.tsx` and `q2-preview/registry.ts`" updated to reflect; no framework edit lands in 2C.
  - **Atomic gate location pinned** at `framework/dispatch.tsx:407-411` — the `effectiveSetLocalAst = isAtomic ? NOOP_SET_LOCAL_AST : setLocalAst` wrap happens inside `<Node>` before format-registry dispatch, so per-type components like `CrossrefResolvedRef.tsx` receive an already-no-op'd `setLocalAst`. Pinned in §Design decisions "Atomic CustomNode read-only — gate inherited from Plan 2B" so future readers don't re-investigate.
  ...
  **Scope additions** (small):
  ...
  - **`ensureHtmlElements` second-array form documented + used**. The runner contract at `parse_ensure_html_elements` accepts `[must_match, must_not_match]`; the smoke fixture spec previously only used must-match. Added must-not-match assertions for `div.q2-preview-placeholder` (no fallback path fired) and `div[data-custom-type]` (no raw CustomNode wrapper leaked through unwrap). Element-and-structure selectors (e.g. `div.callout-note > div.callout-header > div.callout-title-container.flex-fill`) added on the must-match side so the three-deep nesting rule from §"`Callout.tsx`" is locked in the smoke layer.
    ...
    **Sub-fix discovered while wiring the selector**: q2-preview's existing `Block` / `Inline` dispatchers (`q2-preview/dispatchers.tsx`) emit the muted-gray placeholder via inline `placeholderStyle` (no class name). The `div.q2-preview-placeholder` selector would silently match nothing (false-green). Phase 4.4 now bundles a 2-LOC tag-along edit to those existing dispatchers — `className="q2-preview-placeholder"` alongside the existing `style={placeholderStyle}` — so the must-not-match selector actually matches when the placeholder fires.

  **Policy decisions that did NOT change the plan** (recorded for context):
  - **`type_name` block/inline duplication** (e.g. a `Foo` that exists as both CustomBlock and CustomInline) stays unenforced. Codebase posture is "trust the AST shape"; future violations would surface as TS errors inside the per-type component. Same approach as Pandoc's tag namespace.
  - **User overrides of `__fallback__`** stay un-guarded. `customRegistry.test.ts` shows the codebase model is "later silently overrides earlier with no protected keys"; `Block`/`Inline`/`Ast` are also overridable today. Already documented in §Risk areas; no code change.
  - **`renderSlot` user-facing usage example** not added — the calling convention is already established by elliot demos (`const { renderChildren, renderNode } = window.__REACT_AST_DEBUG_RENDERER__`). Adding `renderSlot` is one more key in that destructure.
  - **Slot-kind defensive fallbacks** for non-Equation components were initially flagged as "underspecified" but the research confirmed the codebase's defensive-coding norm is minimal (only `Math.tsx` / `Note.tsx` / `Image.tsx` have visible-failure paths). Equation's three-branch defensive logic is justified by the JS-side `\tag{N}` port being novel surface; the other components matching the codebase's "trust AST shape" posture is on-pattern, not a gap.
