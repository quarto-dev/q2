...
| `RecurseIntoContainer{ before, after }` | editable inside, inline-content block (`has_inline_content`, `inline_plan` present) | R4 (original×inline quadrant) | `Recurse{ shell_open-from-original-prefix, inlines-coarsened-per-`inline_plans`, shell_close-from-original-suffix, separator }`. Falls back to the helper×inline primitive when original-byte shells are unavailable (see "R4 fallback"). |

The dispatch is total, and the rows are read as an **ordered match** (Phase 2 implements `dispatch()` as an explicit ordered match, not a set of mutually-exclusive predicates the reader must reconcile): for a given alignment kind, the first row whose source-info/structure condition holds wins. Totality (Property #2) and the "exactly one row" claim then read off the order. **The row order is not arbitrary — at least one ordering is load-bearing for safety, not just totality:** the `RecurseIntoContainer` "non-editable inside → R1'/R2'" (soft-drop) row **must precede** the "editable inside → R3/R4" rows. Editability-gated kinds (notably non-atomic `Custom` in 7d, see below) rely on matching the soft-drop row *first*; if the rows were reordered so an R3 row matched a `Custom` first, the writer would try to recurse via `custom_node_plans` it does not consume — a panic or silent empty output. Phase 2 must preserve soft-drop-before-recurse ordering and say so in a comment.

**The recursion rules form a 2×2.** Every recursing rule varies on two independent axes — *where the container's shell bytes come from* × *whether the recursed children are blocks or inlines*:

|  | **block children** | **inline children** |
|---|---|---|
| **helper shells** (P2, regenerated by the qmd writer's syntax helpers) | **R3** (dispatches on `block_container_plans`, or no plan under `UseAfter`) | **helper×inline** (`recurse_inline_content_with_helper_shells`, see below) |
| **original-byte shells** (P1, copied from `Source`) | *(unused — a Div edited via `RecurseIntoContainer` regenerates its `:::{.foo}` from helpers, row "editable inside, block container → R3"; it never preserves the source fence bytes — same fidelity class as the documented list-marker/blockquote-prefix normalization)* | **R4** (the inline-splice case; shells are the original block's prefix/suffix bytes, dispatches on `inline_plans`) |

So "R3" and "R4" are not two ad-hoc rules: they are two of the four quadrants. The third used quadrant (helper×inline) is the shared primitive named below; the fourth (original×block) is deliberately unused. This framing makes totality across the recursing rules a property of the grid, not of an enumeration the reader must trust.
...
- `has_inline_content(n)` (`compute.rs:233` — Paragraph, Plain, Header) → **inline-content block**, recurse into inlines (R4 when an `inline_plan` exists, else the helper×inline primitive). **This includes the `UseAfter` Para/Plain/Header case** (brand-new block, no `inline_plan`): the node still routes to inline recursion, where every child inline is itself `UseAfter`/fresh. A Para is *never* serialized whole via R5.
- `is_container_inline(n)` (`compute.rs:759` — Emph, Strong, Link, Span, Cite, Note, …) → **inline container**, inline `Recurse` (this is the *inline-level* dispatch in Phase 3, operating on `Inline` nodes — not to be confused with an inline-content *block*).
- none of the above → **true leaf**, R5.

**Terminology (used consistently below and in the design docs):** an **inline-content block** is a `Block` matching `has_inline_content` (Para/Plain/Header — it *contains* inlines); an **inline container** is an `Inline` matching `is_container_inline` (Emph/Strong/Link — it *is* an inline that contains inlines). The two are different types at different dispatch levels; do not conflate them.

These three predicates are pairwise disjoint by construction (verified: `is_container_block` ∩ `has_inline_content` = ∅; the inline predicate operates on a different type), so the classifier is total and unambiguous. Where the dispatch table above says "block container" read `is_container_block`; "inline-content block" read `has_inline_content`; "true leaf" read "matches none of the three."
...
- [ ] **Add a block-level editability gate.** The opaque-Custom mechanism above relies on a *block-level* "is this node editable-inside?" check that returns `false` for non-atomic `Custom` until 7e. Today only the **inline** equivalent exists (`is_editable_inside_inline`, used at `incremental.rs:1379/1389`); there is no block-level counterpart. 7d must add/extend one (e.g. `is_editable_inside_block`) and wire it into the `RecurseIntoContainer` "non-editable inside → R1'/R2'" dispatch row so non-atomic `Custom` routes to soft-drop ahead of R3. This is the single load-bearing line; everything else about Custom-opacity follows from it + the ordered match.
...
- [ ] Implement the dispatch table from §"The dispatch table" as a single `dispatch(node, align, target) → Rule` function, written as an **explicit ordered match** (first matching row wins) so totality and "exactly one row" are structural, not left to the reader to reconcile predicates. Each rule has a small implementation: R1 packages `Verbatim`; R2 packages `Omit`; R3 / R4 package `Recurse` with shells from Phase 1's helpers and children recursed via `plan_user_writes`; R5 packages `Leaf { serialize_leaf(node) }`. CustomNodes are gated to the opaque (R1'/R2') path until 7e. **Preserve soft-drop-before-recurse row ordering and comment it as load-bearing:** within `RecurseIntoContainer`, the "non-editable inside → R1'/R2'" row must come before the "editable inside → R3/R4" rows, or editability-gated kinds (non-atomic `Custom`) fall into R3 and the writer recurses via a `custom_node_plans` it does not consume. This is a safety ordering, not a totality convenience — a comment must say so to stop a future readability-reorder from breaking it.
...
- [ ] Write a proptest generator `gen_pandoc_with_atomic_descendants` that produces ASTs with atomic-Generated descendants at varying depths inside non-atomic containers, plus arbitrary user edits applied. **The edit model must include an "insert a brand-new paragraph" transformation** — this is what produces a `UseAfter` inline-content block and exercises the helper×inline quadrant; without it the `r3_inline_helper` reachability gate (below) cannot pass.
...
- [ ] **Hand-written fixture for the R4 `None`-fallback path** (do not rely on the random generator). The helper×inline quadrant has two triggers — `UseAfter`-Para (easily generated) and the R4 fallback when `assemble_inline_splice` returns `None` (needs a non-contiguous `Concat` / anchorless `Generated` boundary, which the random generator is unlikely to produce). The single `r3_inline_helper` reachability counter is satisfied by the easy trigger, leaving the fallback branch — a rare path where bugs hide — uncovered. Add one deterministic fixture that constructs such a boundary and asserts the fallback routes to `recurse_inline_content_with_helper_shells` (not the deleted `Rewrite`, not a panic). This mirrors how `r5_special` is covered out-of-band rather than via the generator; do **not** split the counter and demand the generator reach the fallback (that would force a new generator capability for no benefit).
...
- [ ] **Sync the recursion-rule set with the design docs.** The contract (`incremental-writer-contract.md` §"The dispatch") enumerates "five rules R1–R5" and the BP proof's L2 lemma says "only R3/R4 recurse." The helper×inline quadrant (`recurse_inline_content_with_helper_shells`) is a *third* recursing rule. Add a one-line note to each: it is an R3-class recursing rule (so L2's "preimage-emission is terminal; only recursing rules descend" set must include it) whose shells are **P2** (helper-derived), so it stays *out* of the (M) disjoint P1-copy family. No proof step changes — the induction already covers "recurse with P2 shells"; only the enumeration needs the addition.
