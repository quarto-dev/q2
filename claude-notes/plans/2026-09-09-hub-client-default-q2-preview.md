...
| toc-containers/div-heading-becomes-section | `blockquote > h4#quoted` | bd-q2wqj24c — **re-scoped 2026-09-10**: sectionize is correct; the live DOM is `blockquote > div[style="position: relative; box-shadow…"] > h4`. The div is `CommentBlock`'s per-block wrapper (present in read-only `q2 preview` too: 15 wrappers in this doc), which the parity harness never sees. It breaks every direct-child theme rule (`.callout-body > :first-child`, `li > p:last-of-type`, `.tab-pane > p`, …). **Correction:** `display: contents` does not help (`>` matches the DOM tree). The wrapper element must go — overlay layer + host-ref contract. **Done 2026-09-10** (plan `2026-09-10-commentblock-wrapper-removal.md`): `CommentBlock` renders the block untouched and portals its chrome into one body-level overlay layer positioned from the block's rect; block components register their host element through `CommentAnchorContext`, tight-list `Plain`s anchor to their `PlainHost` `<li>`. Fixture unlisted and green in smoke-all e2e; `.callout-body > :first-child` and `blockquote > h4` now compute the same margins as `q2 render`. |
