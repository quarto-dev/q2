...
## ⚠ Integration point flagged early — outermost-container clean buffer (READ FIRST)

This is the single most load-bearing integration question this rewrite surfaces; it must be resolved with Plan 1 (and possibly Plan 2/backend) before Phase 2 code lands.
...
- (Only if the flagged integration point resolves to option (A)) a small `pampa` extension emitting the outermost-container clean buffer + its `regenerate_nested_buffers_tests` backstop.
...
- [ ] **0.2a (the buffer integration point — RESOLVE before any Phase-2 code).** Confirm `ViewControllerProps.editBufferCache.editableTextFor(node)` exists and decide the **outermost-container** case (see the flagged section above). Empirically, with a fixture `> quote line one\n> quote line two`:
  ...
  - If it returns the polluted raw slice → coordinate with Plan 1: prefer **option (B)** (the cache owns the outermost-container generated case, per keystone §7.1's "single shared predicate"); only if Plan 1 directs, take **option (A)** (extend `regenerate_nested_buffers_ast` to emit the container's own buffer) — and then **flag to the user that Plan 5 now includes a `pampa` Rust change** (scope widened; full WASM gate + TDD backstop). Record the chosen option + the owner in Notes. **Do NOT proceed to Phase 2 until the de-prefixed buffer for an outermost blockquote/list is verifiably available.**
...
- **Outermost-container buffer integration point is unresolved at plan-write time.** The current `regenerate_nested_buffers_ast` emits buffers for *prefixed descendants*, not the *container itself* (grounded read: `crates/pampa/src/regenerate_nested_buffers.rs:42-203`). Phase 0.2a decides A/B/C with Plan 1; option (A) widens scope to a `pampa` Rust change (flag to user). Prefer option (B) (cache owns the outermost-container generated case) per keystone §7.1's "single shared predicate". **Do not start Phase 2 until the de-prefixed outermost buffer is verifiably available.**
