...
- [x] **C3 — Wire minting + sliding re-issue.** *(done 2026-07-24,
  `bd-nh5pt1pd`. `auth_callback`/`auth_refresh` validate the Google token
  once then mint via `mint_session_cookie` (`auth_time = now`, fresh `sid`,
  identity from validated Google claims with defensive `name`/`picture` caps
  in `SessionIdentity::from_oidc`); cookie `Max-Age` now follows the token's
  sliding exp (`AUTH_COOKIE_MAX_AGE` removed). `auth_me`/`auth_actor` route
  through shared `authenticate_request` (dual-credential 400 applies; Bearer
  accepted — implements `bd-3g0aijb3`). Sliding re-issue is a response layer
  (`session_reissue_layer`, `middleware::from_fn_with_state`, wraps all
  routes): skips when any Authorization header is attached, on non-2xx (incl.
  WS 101), or when the handler already set a cookie (login mint / logout
  clear); re-validates the token itself incl. the allowlist re-check before
  extending; re-issues at ≥ 1 h age or non-current `kid`; `auth_time`
  immutable, exp capped. 10 new integration tests written first and observed
  failing, then green — incl. survives-past-1 h, large-Google-token, cookie
  attributes (Secure preserved on secure hub), and all never-re-issue
  cases.)* `auth_callback` + `auth_refresh` validate the Google token once, then mint the session cookie (`auth_time = now`, fresh random `sid`, `email_verified` stamped from the validated Google claims); `auth_me`/`auth_actor` switch to session verify — and, while rewiring them, route them through the shared credential extraction rather than entrenching today's `cookie_token`-only bypass of the dual-credential rule (coordinate with the standalone `bd-3g0aijb3`); re-issue `Set-Cookie` per the §2 constraints (Cookie-kind only, post-full-validation, ≥ 1 h token age or non-current `kid`, `auth_time` immutable, `exp` capped). Tests: session survives past 1 h with no One-Tap; `/auth/me` returns sliding `exp`; large Google token no longer cookie-dropped; re-issue never fires on Bearer responses; re-issued cookie preserves attributes (`HttpOnly`, `SameSite=Lax`, `Secure`, `Path=/`); re-issued token keeps the original `auth_time`.
- [x] **C4 — Invariants + hard-break cutover + security review.** *(done
  2026-07-24, `bd-74iiwb3l`. Hard break verified on every cookie surface:
  legacy Google-JWT cookie → 401 with clean JSON logged-out body on
  `/health`, `/auth/me`, and the WS upgrade; callback failure → single
  redirect to `/?auth_error`, no cookie, no loop. Algorithm-confusion pinned
  (`rs256_with_hub_kid_rejected`); dual-credential 400 + cookie-only
  CSRF/WS-Origin re-verified with session cookies. Dead legacy-accept API
  removed (`HubContext::authenticate`, zero callers) and
  `authenticate_claims` re-documented as the mint-time/Bearer Google
  verifier. Reviewed against the device-flow Phase 2 auth-confusion
  protections; the confusion-matrix invariants are pinned by the cross-path,
  dual-credential, and CSRF/Origin tests.)* Preserve dual-credential 400, cookie-only CSRF/Origin; **hard break for legacy Google-JWT cookies (§6)** — no dual-accept window, no legacy-accept code; review the token format against the auth-confusion protections that drove device-flow Phase 2. Tests: legacy Google-JWT cookie → 401 and a clean logged-out flow (no redirect loop); CSRF/Origin gates intact.
- [x] **C5 — Revocation-event store.** *(done 2026-07-24, `bd-3dq0x6ut`.
  New `crates/quarto-hub/src/revocation.rs`: `RevocationLedger` —
  `revocations.json` (never `hub.json`) with a per-`sub` `not_before`
  `BTreeMap` + `banned` `BTreeSet` (BTree for deterministic, hand-editable
  serialization), tokio-mutex in-memory state, atomic temp+rename persist
  (0o600), loaded at startup with GC (logout entries expire past the
  absolute cap; bans never GC'd; a malformed file is a startup error — never
  silently un-ban). `authenticate_session` rejects banned (403,
  `user_banned`) and revoked (`auth_time < not_before` → 401,
  `session_revoked`); both mint points refuse a banned `sub`; the re-issue
  layer shares the policy via `session_policy_ok` (revoked/banned tokens are
  never extended). `POST /auth/logout-everywhere`: cookie-kind only, CSRF-
  gated, dual→400, kills the whole token family (re-issued siblings
  included), clears the caller's cookie, immediate re-login works
  (`auth_time ≥ not_before`; `not_before` never moves backwards). Fixture
  builder's `banned_subs` pre-writes `revocations.json` = the documented
  stopped-hub operator ban procedure, so the restart-enforcement path is
  covered. 8 ledger unit tests + 3 integration tests, written first.)* Dedicated `revocations.json` per §3: in-memory per-`sub` `not_before` map + ban entries behind a tokio mutex, atomic temp+rename persist, loaded at startup; verify rejects `auth_time < not_before[sub]` or banned `sub`; **mint refuses a banned `sub`** (`auth_callback`/`auth_refresh`); self-service `POST /auth/logout-everywhere` (CSRF-gated, cookie-kind only); GC on load/write (logout entries expire after the absolute cap; **bans never GC'd**). Own PR sequenced after C4 — keeps the minting release reviewable. Tests: logout-everywhere → prior tokens (including re-issued siblings) rejected, immediate re-login works (`auth_time ≥ not_before`); ban → verify rejected AND mint refused; revocations + bans survive restart; expired logout entries GC'd, bans retained; writes are atomic and never touch `hub.json`; endpoint requires CSRF header and rejects Bearer-kind callers; a ban entry hand-added to `revocations.json` while the hub is stopped is enforced after restart (the documented ban procedure, §3).
- [x] **C5b — Secret rotation via `kid` overlap.** *(done 2026-07-24,
  `bd-6kll0jr6`. `previous_session_secret` + `session_secret_rotated_at` in
  `hub.json` (env `QUARTO_HUB_SESSION_SECRET_PREVIOUS` +
  `QUARTO_HUB_SESSION_SECRET_ROTATED_AT`, sources strictly paired);
  `resolve_previous_session_secret` loads the previous key only inside the
  overlap window (`rotated_at + idle > now`), auto-dropping it after; a
  previous secret **without** its rotated-at timestamp is a hard startup
  error (an unbounded window would silently defeat the rotation). Keyring
  stays ≤ 2 by construction (`SessionKeys::with_previous`; kid-collision
  warning). Old-kid cookies verify during overlap and are promptly re-minted
  under the new kid on their next request (§2c — non-current kid overrides
  the 1 h age gate; session continuity: same `auth_time`/`sid`); new logins
