...
- Plan 2B — framework recursion + atomic gate that the IncludeExpansion
  component runs through; CustomNode unwrap/rewrap walks that produce
  the JS-native shape Plan 2C's component consumes.
...
- **Plan 2C component test**: render an IncludeExpansion wrapper; assert
...
- Plan 2C also depends on this for the IncludeExpansion component (which
  Plan 8 confirms is needed; Plan 2C ships the placeholder dormant).
