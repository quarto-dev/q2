...
- [x] **0.1** Run the slide-side rename on a throwaway branch per §"Phase 0: Pre-flight." Discard the branch.

  **Result (2026-05-07):** PASSES. `npm run build:all` reported only TS6196 "declared but never used" errors for per-shape types (`HorizontalRuleBlock`, `UnknownBlock`, `SpaceInline`, `SoftBreakInline`, `LineBreakInline`, `UnknownInline`) that were referenced exclusively by the dropped union declarations. **None** of the anticipated narrowing failures materialized: the framework's narrower `Quoted` discriminant compiled fine; the missing `MathInline` did not surface (slide-side keeps a local `MathInline` and the `case 'Math'` branch uses `inline as MathInline`, with `UnknownInline = { t: string }` accepting `'Math'` at the switch entry); no `splitByHeaders` / `extractSections` / `flattenBlocks` boundary failures. The unused-type cleanup falls naturally into Phase 2.2 when slide-side moves to importing per-shape types from `framework/types`.
...
**Bundled commit decision (2026-05-07).** Phase 1 was committed as **a single squashed commit** rather than the originally-planned 11. The shim isolates consumers from the moves, so Phase-1's intermediate states are inert (new files unreferenced) until step 1.10 wires them in via the shim — there is no debugging value in landing the intermediate steps separately, and a single commit reads more cleanly. Phase 2's per-step commit cadence is preserved.

- [x] **1.1** Create `framework/types.ts`. Move `PandocAST`, all Block variant types and `BlockNode` union, all Inline variant types and `InlineNode` union (**including the new `MathInline` variant**), `NodeArgs<T>`. Add `AstProps`, `AstComponent`, `DispatcherComponent`, `FormatRegistry` (§"Typed format-registry contract").
- [x] **1.2** Create `framework/RegistryContext.tsx`. Default value `{ registry: {} }` (no `null`).
...
- [x] **1.4** Create `framework/Ast.tsx`. `registry` prop required (no default). Drop the `?? componentRegistry` prop default.
- [x] **1.5** Create `framework/index.ts`. Re-export everything in `types.ts`, plus `RegistryContext`, `Ast`, `Node`, `renderChildren`, `renderNode`, `blockTypes`. Do **not** re-export `renderChildrenRegistry`.
- [x] **1.6** Create `q2-debug/styles.ts`. Move `blockStyle`, `inlineStyle`.
- [x] **1.7** Create `q2-debug/dispatchers.tsx`. Move `Block` and `Inline` dispatchers. Drop the `?? componentRegistry` fallback at both sites.
...
- [x] **2.15** Final full verification: `cargo xtask verify --skip-rust-tests` end-to-end (Rust workspace + hub-client build + `test:ci` + e2e with reactji fixture). Update `hub-client/changelog.md` per the project's hub-client commit instructions.
