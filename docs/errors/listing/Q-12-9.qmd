...
$endfor$`; `<% if (item.image) { %>` becomes `$if(it.image)$`.
[`Q-12-24`](Q-12-24.qmd) has a fuller mapping table, and the
[Listings guide](/guides/projects/listings.qmd#custom-templates)
lists the values a template can read.

That syntax translation is the mechanical half of the port. The half
that fails silently is link and image markup. A doctemplate's output
is markdown, and Quarto resolves paths only after parsing it — so a
link written as raw HTML keeps its `.qmd` href, and an image written
as raw HTML is never copied into the site at all. Neither failure
produces a warning. [Listing
templates](/guides/projects/listing-templates.qmd) covers both, and is
worth reading before you port rather than after.
