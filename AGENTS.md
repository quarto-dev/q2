...
- **`AGENTS.md`** — always-on guidance, project conventions, commands.
...
- It duplicates something already in `AGENTS.md` or a plan/research
  note.
...
scheduled `nightly.yml`) is deliberately outside this mirror: it
publishes rather than gates, and its own gates run on every nightly
(see `claude-notes/instructions/release-runbook.md` § Nightlies).
