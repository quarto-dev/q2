...
fn test_unicode_error_position() {
    ...
    let result = readers::qmd::read(input_bytes, false, "test.qmd", &mut output);

    // Should have errors (the '#' is invalid)
    assert!(result.is_err(), "Expected parse errors for invalid '#' character");
    ...
    assert!(
        error_text.contains("2:2") || error_text.contains("line 2") && error_text.contains("column 2"),
