...
pub(crate) fn extract_title_inlines(meta: &ConfigValue) -> Option<Vec<Inline>> {
...
pub(crate) fn create_title_header(content: Vec<Inline>) -> Block {
    let source_info = SourceInfo::generated(By::title_block());
...
mod tests {
    ...
    async fn test_minimal_mode_preserves_inline_markup_in_title() {
        ...
        assert!(
            matches!(&h.source_info, SourceInfo::Generated(g) if g.by.is_kind("title-block")),
            "header must be generated-by-title-block"
        );
        for child in &h.content {
            assert!(
                matches!(child.source_info(), SourceInfo::Generated(g) if g.by.is_kind("title-block")),
