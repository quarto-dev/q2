...
-- QUARTO2-PATCH (q2 pandoc-hybrid, P4): imports the placeholder shim that
-- defines quarto_pandoc_shim_filters, spliced into quarto_filter_list
-- between quarto_init_filters and quarto_normalize_filters below -- see
-- resources/pandoc-filters/README.md. Must come after the
-- ast/customnodes.lua and customnodes/*.lua imports above, which define
-- what the shim will call once P5 fills it in.
import("./quarto2-shim.lua")
...
-- QUARTO-PATCH(upstream PR quarto-dev/quarto-cli#14913): whether crossref
-- numbers are presented, and whether quarto itself assigns them, are now
-- separate questions: see _quarto.modules.crossref_numbering
-- (crossref_present / assign_crossref_numbers).
...
-- QUARTO2-PATCH (q2 pandoc-hybrid, P4): splices P5's wire-format shim
-- between quarto_init_filters and quarto_normalize_filters -- see
-- resources/pandoc-filters/README.md. Must run after quarto_init_filters
-- (crossrefOption() indexes crossref.options, nil until
-- init_crossref_options(meta) runs inside that group) and before
-- quarto_normalize_filters (the wire Div retains its original semantic
-- classes, and Q1's class-keyed dispatcher in that group would otherwise
-- fire Callout.parse()/Tabset.parse()/ConditionalBlock.parse() on it).
...
-- QUARTO-PATCH(upstream PR quarto-dev/quarto-cli#14913): fail fast when
-- crossref-numbering: external is combined with a LaTeX-family or Typst
-- target, before Pandoc's writer sees malformed output.
...
-- QUARTO-PATCH(upstream PR quarto-dev/quarto-cli#14913): convert the
-- enableCrossRef gate to assign_crossref_numbers().
