/**
 * BreadcrumbChip.tsx — floating ancestor-path breadcrumb for the depth cursor.
 *
 * P3.4 §3d: shows the AST ancestor path (e.g. "Section › Div › Paragraph")
 * with the current level highlighted. Rendered when unlockDepthCursor=true
...
import { buildAncestorPath, detectPlatform } from './depthNav';
...
export function BreadcrumbChip(): React.ReactElement | null {
    ...
    const active = !!ctx?.unlockDepthCursor && !!et;
    ...
    return (
        ...
        >
            <button
                ...
                onClick={(e) => { e.stopPropagation(); ctx?.requestDepthMove?.('out'); }}
            >◀</button>
            {crumbs.map((c, i) => (
                <React.Fragment key={`${c.r0}-${c.r1}`}>
                    ...
                    <button
                        ...
                        onPointerDown={(e) => e.preventDefault()}
                        onClick={(e) => { e.stopPropagation(); ctx?.requestDepthSelect?.(c.r0, c.r1); }}
                    >{c.label}</button>
                </React.Fragment>
            ))}
            <button
                ...
                aria-label={inTip}
                onPointerDown={(e) => e.preventDefault()}
                onClick={(e) => { e.stopPropagation(); ctx?.requestDepthMove?.('in'); }}
            >▶</button>
        </div>
    );
}
