/**
 * Equivalence invariant: buildNestingCommitDestination vs. JSON.stringify(resolved.sourceEntry)
 *
 * This test guards the safety premise of the "live-identity refactor" (P3.3 §3 hardening):
 * for every editable block, the commit destination produced by buildNestingCommitDestination
 ...
 *
 * buildNestingCommitDestination returns JSON.stringify({ t: 0, r: [anchorR0, anchorR1], d: 0 }).
...
describe('commit-destination equivalence invariant', () => {
    it('buildNestingCommitDestination === JSON.stringify(resolved.sourceEntry) for a representative editable block', () => {
        ...
        const liveForm = buildNestingCommitDestination({ anchorR0: 6, anchorR1: 12 });

        expect(liveForm).toBe(closureForm);
    });

    it('buildNestingCommitDestination === JSON.stringify(resolved.sourceEntry) for a nested editable block', () => {
        // Nested child para from the blockquote fixture: {t:0, r:[2,22], d:0}
        const sourceEntry = { t: 0, r: [2, 22] as [number, number], d: 0 };

        const closureForm = JSON.stringify(sourceEntry);
        const liveForm = buildNestingCommitDestination({ anchorR0: 2, anchorR1: 22 });

        expect(liveForm).toBe(closureForm);
    });

    it('buildNestingCommitDestination === JSON.stringify(resolved.sourceEntry) for a top-level block at byte 0', () => {
        // Top-level block at the start of the document: {t:0, r:[0,6], d:0}
        const sourceEntry = { t: 0, r: [0, 6] as [number, number], d: 0 };

        const closureForm = JSON.stringify(sourceEntry);
        const liveForm = buildNestingCommitDestination({ anchorR0: 0, anchorR1: 6 });

        expect(liveForm).toBe(closureForm);
    });

    it('returns null when there is no active edit target', () => {
        // Null-guard: if editTargetRef.current is null, buildNestingCommitDestination returns null.
