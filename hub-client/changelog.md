...
- [`d2a6ca77`](https://github.com/quarto-dev/q2/commits/d2a6ca77): Plan 2B Phase 2 — asset manifest plumbing for q2-preview. Parent-side walker (`q2-preview/assetWalker.ts::buildAssetManifest`) walks the AST for `Image` nodes, resolves paths via the now-shared `vfsPaths.ts`, reads VFS bytes, and mints (or reuses cached) blob URLs. Cache identity is the base64 content string itself — synchronous, deterministic, no hash. External URLs (`http(s):`, `data:`, `//`) are filtered out at walk time. New `q2-preview/AssetManifestContext.tsx` distributes `{ origPath → blobUrl }` to iframe consumers. `Q2PreviewIframe.tsx` owns the cache (drained on unmount) and extends the `UPDATE_AST` payload to `{ astJson, currentFilePath, assetManifest }` (manifest piggybacks on the AST so they always arrive together). `entry.tsx`'s `PreviewRoot` wraps `<Ast>` with `<AssetManifestContext.Provider>` alongside the existing `PreviewContext`; the `setAst` callback now `rewrapCustomNodes()`s the JS-native AST back to wire format before posting `SET_AST`. Walker test suite covers cache hit, content-change eviction with revocation, image removal, external-URL skipping, VFS failure, N=100 stress, malformed JSON, and nested-container traversal (Figure / BulletList / Div). Q2PreviewIframe integration test extended with a third case asserting the manifest in the `UPDATE_AST` payload + unmount revocation. 688 unit + 90 integration tests pass.
- [`f3c32926`](https://github.com/quarto-dev/q2/commits/f3c32926): Plan 2B Phase 2.1a — extract `hub-client/src/utils/vfsPaths.ts`. `resolveRelativePath`, `normalizePath`, and `guessMimeType` previously had three private copies (`iframePostProcessor.ts`, `iframeLinkHandlers.ts`, `ReactAstSlideRenderer.tsx`); consolidating to a shared module avoids fanning the duplication out further when Plan 2B's asset walker becomes the fourth consumer. Pure refactor — no behavior change. New module includes 13 unit tests covering absolute-path passthrough, `..` traversal, `.` segments, empty segments, the single-leading-slash invariant, past-root `..` swallowing, image and text MIME types, and the octet-stream fallback.
