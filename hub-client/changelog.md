...
- [`6510577c`](https://github.com/quarto-dev/q2/commits/6510577c): Internal — move the q2-preview format components (~50 files), iframe wrappers, and overlay views from `hub-client/src/components/render/` into `@quarto/preview-renderer` (bd-hfjj Phase 4, the largest single move in the decomposition). `PreviewErrorOverlay`'s collapsed-state plumbing was DI-refactored so the overlay no longer reads `localStorage` directly; hub-client wraps with `usePreference` at the two call sites, preserving the existing UX. No user-visible change.
