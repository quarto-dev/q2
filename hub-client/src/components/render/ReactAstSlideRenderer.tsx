...
/**
 * Render a Pandoc `RawBlock(html, …)` so embedded `<script>` tags
 * actually execute, matching the static-render behaviour.
 *
 ...
 * AST passes on their output, diagram engines like
 * [`MermaidEngine`](../../../../crates/quarto-core/src/engine/mermaid.rs)
 * emit marker nodes that the preview renderer can handle with
 * dedicated, script-free React components, removing the need for this
...
export function renderBlock(
  ...
  switch (block.t) {
    ...
    case 'RawBlock': {
      ...
      if (format === 'html') {
        return <RawHtmlBlock key={key} content={content} />;
      }
      return null;
    }
