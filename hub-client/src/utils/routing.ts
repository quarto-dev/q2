...
export interface ShareRoute {
  ...
  name: string;
  /**
   * True only on boot URLs generated by `q2 preview --ui editor`: the
   * serving hub is an ephemeral per-session server, so the app skips
   * project-set onboarding and silently establishes a root set against
   * it (bd-zf4ryvuq). Never emitted by buildHashRoute — production
...
export function parseHashRoute(hash: string): Route {
  ...
  // `ephemeral=true` is optional and marks a `q2 preview` boot URL.
