...
### Path resolution is a bug *class* — check the contract, not just the reported key

Config-authored paths (`_quarto.yml`, `_metadata.yml`, front matter) resolve **relative to the file that declared them**; a leading `/` means the project root, never the filesystem root. Historically, each consumer chose its own base directory ad hoc, so the same defect recurs key by key (`css:` was fixed while `include-in-header:` in the same YAML block stayed broken — GH #455 vs PR #524). **Before scoping any path-resolution bug or fix, read the normative contract `claude-notes/designs/path-resolution-model.md`** and check the sibling keys in its consumption-site inventory; a consumer must never `join` a config string onto a base it picked itself. Deliberate scope-outs must be filed as strands linked to bd-oejuizi9 before merging. When auditing, enumerate *consumers* (file reads / joins), not implementations of the convention — grepping for leading-`/` handlers cannot find the sites that fail to handle it.
